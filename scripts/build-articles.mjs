...
fs.writeFileSync(path.join(OUT,'index.html'),`<!doctype html><html lang="ko"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>레트로게임 가이드 | 게임다방</title><meta name="description" content="도스게임, 오락실게임, 메가드라이브, PS1 추천과 초보 공략을 모은 게임다방 정보 가이드입니다."><link rel="canonical" href="${SITE}/articles/"><link rel="icon" type="image/svg+xml" href="../favicon.svg"><link rel="apple-touch-icon" href="../icon-192.png"><meta property="og:type" content="website"><meta property="og:title" content="레트로게임 가이드 | 게임다방"><meta property="og:description" content="도스게임, 오락실게임, 메가드라이브, PS1 추천과 초보 공략을 모은 게임다방 정보 가이드입니다."><meta property="og:url" content="${SITE}/articles/"><meta property="og:image" content="${OG_IMAGE}"><meta property="og:site_name" content="게임다방"><meta property="og:locale" content="ko_KR"><meta name="twitter:card" content="summary_large_image"><script type="application/ld+json">${JSON.stringify({"@context":"https://schema.org","@type":"BreadcrumbList",itemListElement:[{"@type":"ListItem",position:1,name:"홈",item:SITE+"/"},{"@type":"ListItem",position:2,name:"레트로게임 가이드",item:SITE+"/articles/"}]})}</script><style>${css}</style></head><body><main class="wrap">${nav}<h1>📚 레트로게임 가이드</h1><p class="lead">추천작, 조작법, 초보 공략을 읽고 바로 게임을 시작하세요.</p><div class="grid">${cards}</div></main></body></html>`);

const sitemapPath = path.join(ROOT,'sitemap.xml');
let sitemap = fs.readFileSync(sitemapPath,'utf8');
sitemap = sitemap.replace(/\n<url><loc>https:\/\/bamboostand\.kr\/articles\/[\s\S]*?<\/url>/g,'');

// Same honest-lastmod store build-pages.mjs uses: the date advances only when
// the rendered page changes.
const LASTMOD_PATH = path.join(ROOT,'scripts','lastmod.json');
let lastmodDb = {};
try { lastmodDb = JSON.parse(fs.readFileSync(LASTMOD_PATH,'utf8')); } catch { lastmodDb = {}; }
const buildDate = new Date().toISOString().slice(0,10);
function lastmodFor(url, file){
  const html = fs.readFileSync(file,'utf8');
  const hash = crypto.createHash('sha1').update(html).digest('hex').slice(0,16);
  if(!lastmodDb[url] || lastmodDb[url].hash !== hash) lastmodDb[url] = { hash, date: buildDate };
  return lastmodDb[url].date;
}
const articleUrls = [
  [`${SITE}/articles/`, path.join(OUT,'index.html')],
  ...articles.map(a=>[`${SITE}/articles/${a.slug}.html`, path.join(OUT,`${a.slug}.html`)]),
];
sitemap = sitemap.replace('</urlset>', `${articleUrls.map(([u,f])=>`\n<url><loc>${u}</loc><lastmod>${lastmodFor(u,f)}</lastmod></url>`).join('')}\n</urlset>`);
